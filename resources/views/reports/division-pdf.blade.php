...
<head>
    ...
    <title>{{ $division->title }} Report - {{ $date }}</title>
    <style>
        body {
...
<body>
    <div class="header">
        <h1>{{ $division->title }} Report</h1>
        <h2>{{ $division->description ?? 'Division Report' }}</h2>
    </div>
    
    <div class="date-info">
        <strong>Report Date:</strong> {{ \Carbon\Carbon::parse($date)->format('l, d F Y') }}
        @if($subdivision)
        <br><strong>Sub Division:</strong> {{ $subdivision->title }}
        @endif
    </div>
    ...
    @if(count($reportData) > 0)
        ...
        <div class="employee-section">
            ...
            @else
            <div class="no-data">
                No logs found for this employee on {{ \Carbon\Carbon::parse($date)->format('d F Y') }}
            </div>
            @endif
        </div>
        @endforeach
    @else
    <div class="no-data">
        No data found for {{ \Carbon\Carbon::parse($date)->format('d F Y') }}
    </div>
    @endif
    
    <div class="footer">
        <p>Generated on {{ now()->format('d F Y H:i:s') }} | {{ $division->title }} Daily Log System</p>
    </div>
</body>
</html>
