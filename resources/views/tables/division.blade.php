...
@section('title', 'Division - Daily Log System')
...
@section('content')
<div class="pagetitle">
    <h1 class="text-capitalize">Division</h1>
</div>
    ...
    <div class="row">
        <div class="card">
            <div class="card-body pt-3">
                <h4 class="text-capitalize mb-4">Manage Division</h4>
                <div class="d-flex align-items-center justify-content-between mb-3">
                    <div class="d-flex gap-2">
                        <button type="button" id="btn-add" class="btn btn-sm btn-primary"><i class="bi bi-plus-lg"></i>&nbsp;Add Division</button>
                    </div>
                    <div class="search-bar">
                        <div class="search-data d-flex align-items-center">
                            <input type="text" id="search" class="search" placeholder="Search Division" title="Enter search keyword">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="divisionForm">
                    ...
                    <div class="mb-3">
                        <label for="division-title" class="form-label">Division Name <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" id="division-title" required>
                    </div>
                    <div class="mb-3">
                        <label for="division-description" class="form-label">Description</label>
                        <textarea class="form-control" id="division-description" rows="3"></textarea>
                    </div>
                </form>
            </div>
...
<script>
    ...
    function loadData() {
        ...
        fetch(`{{ url('/division/getData') }}?data=${encodeURIComponent(JSON.stringify(params))}`)
    ...
    function renderTable(divisions) {
        ...
        tbody.innerHTML = divisions.map((div, index) => `
            <tr>
        ...
        document.querySelectorAll('.btn-edit').forEach(btn => {
            btn.addEventListener('click', function() {
                editDivision(this.dataset.id, this.dataset.title, this.dataset.description);
            });
        });

        document.querySelectorAll('.btn-delete').forEach(btn => {
            btn.addEventListener('click', function() {
                deleteDivision(this.dataset.id);
            });
        });
    }
    ...
    // Edit division
    function editDivision(id, title, description) {
        document.getElementById('divisionModalLabel').textContent = 'Edit Division';
        document.getElementById('division-id').value = id;
        document.getElementById('division-title').value = title;
        document.getElementById('division-description').value = description;
        new bootstrap.Modal(document.getElementById('divisionModal')).show();
    }
    ...
    document.getElementById('btn-save').addEventListener('click', function() {
        ...
        const title = document.getElementById('division-title').value;
        ...
        const url = id ? `{{ url('/division') }}/${id}` : '{{ url('/division') }}';
        ...
        .then(data => {
            if (data.success) {
                ...
                Swal.fire({
                    title: 'Success',
                    text: data.message || 'Division saved successfully',
                    icon: 'success',
                    confirmButtonColor: '#0d6efd'
                });
            } else {
                Swal.fire({
                    title: 'Error',
                    text: data.message || 'Error saving division',
                    icon: 'error',
                    confirmButtonColor: '#0d6efd'
                });
            }
            ...
            if (result.isConfirmed) {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        Swal.fire({
                            title: 'Deleted!',
                            text: data.message || 'Division deleted successfully',
                            icon: 'success',
                            confirmButtonColor: '#0d6efd'
                        });
                    } else {
                        Swal.fire({
                            title: 'Error',
                            text: data.message || 'Error deleting division',
                            icon: 'error',
                            confirmButtonColor: '#0d6efd'
                        });
                    }
