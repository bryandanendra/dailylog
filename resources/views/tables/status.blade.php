...
@section('title', 'Status - Daily Log System')
...
@section('content')
<div class="pagetitle">
    <h1 class="text-capitalize">Status</h1>
</div>
    ...
    <div class="row">
        <div class="card">
            <div class="card-body pt-3">
                <h4 class="text-capitalize mb-4">Manage Status</h4>
                <div class="d-flex align-items-center justify-content-between mb-3">
                    <div class="d-flex gap-2">
                        <button type="button" id="btn-add" class="btn btn-sm btn-primary"><i class="bi bi-plus-lg"></i>&nbsp;Add Status</button>
                    </div>
                    <div class="search-bar">
                        <div class="search-data d-flex align-items-center">
                            <input type="text" id="search" class="search" placeholder="Search Status" title="Enter search keyword">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="statusForm">
                    ...
                    <div class="mb-3">
                        <label for="status-title" class="form-label">Status Name <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" id="status-title" required>
                    </div>
                    <div class="mb-3">
                        <label for="status-description" class="form-label">Description</label>
...
<script>
    ...
    function loadData() {
        ...
        fetch(`{{ url('/status/getData') }}?data=${encodeURIComponent(JSON.stringify(params))}`)
    ...
    function renderTable(statuss) {
        ...
        tbody.innerHTML = statuss.map((div, index) => `
            <tr>
        ...
        document.querySelectorAll('.btn-edit').forEach(btn => {
            btn.addEventListener('click', function() {
                editStatus(this.dataset.id, this.dataset.title, this.dataset.description);
            });
        });

        document.querySelectorAll('.btn-delete').forEach(btn => {
            btn.addEventListener('click', function() {
                deleteStatus(this.dataset.id);
            });
        });
    }
    ...
    document.getElementById('btn-add').addEventListener('click', function() {
        document.getElementById('statusModalLabel').textContent = 'Add Status';
    ...
    // Edit status
    function editStatus(id, title, description) {
        document.getElementById('statusModalLabel').textContent = 'Edit Status';
        document.getElementById('status-id').value = id;
        document.getElementById('status-title').value = title;
        document.getElementById('status-description').value = description;
        new bootstrap.Modal(document.getElementById('statusModal')).show();
    }
    ...
    document.getElementById('btn-save').addEventListener('click', function() {
        ...
        const url = id ? `{{ url('/status') }}/${id}` : '{{ url('/status') }}';
        ...
        .then(data => {
            if (data.success) {
                ...
                Swal.fire({
                    title: 'Success',
                    text: data.message || 'Status saved successfully',
                    icon: 'success',
                    confirmButtonColor: '#0d6efd'
                });
            } else {
                Swal.fire({
                    title: 'Error',
                    text: data.message || 'Error saving status',
                    icon: 'error',
                    confirmButtonColor: '#0d6efd'
                });
            }
            ...
            if (result.isConfirmed) {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        Swal.fire({
                            title: 'Error',
                            text: data.message || 'Error deleting status',
                            icon: 'error',
                            confirmButtonColor: '#0d6efd'
                        });
                    }
