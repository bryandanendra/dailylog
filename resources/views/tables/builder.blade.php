...
@section('title', 'Builder - Daily Log System')
...
@section('content')
<div class="pagetitle">
    <h1 class="text-capitalize">Builder</h1>
</div>
    ...
    <div class="row">
        <div class="card">
            <div class="card-body pt-3">
                <h4 class="text-capitalize mb-4">Manage Builder</h4>
                <div class="d-flex align-items-center justify-content-between mb-3">
                    <div class="d-flex gap-2">
                        <button type="button" id="btn-add" class="btn btn-sm btn-primary"><i class="bi bi-plus-lg"></i>&nbsp;Add Builder</button>
                    </div>
                    <div class="search-bar">
                        <div class="search-data d-flex align-items-center">
                            <input type="text" id="search" class="search" placeholder="Search Builder" title="Enter search keyword">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="builderModalLabel">Add Builder</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="builderForm">
                    <input type="hidden" id="builder-id">
                    <div class="mb-3">
                        <label for="builder-title" class="form-label">Builder Name <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" id="builder-title" required>
                    </div>
                    <div class="mb-3">
                        <label for="builder-description" class="form-label">Description</label>
                        <textarea class="form-control" id="builder-description" rows="3"></textarea>
                    </div>
                </form>
            </div>
...
<script>
    ...
    function loadData() {
        ...
        fetch(`{{ url('/builder/getData') }}?data=${encodeURIComponent(JSON.stringify(params))}`)
    ...
    function renderTable(builders) {
        ...
        tbody.innerHTML = builders.map((div, index) => `
            <tr>
        ...
        document.querySelectorAll('.btn-edit').forEach(btn => {
            btn.addEventListener('click', function() {
                editBuilder(this.dataset.id, this.dataset.title, this.dataset.description);
            });
        });

        document.querySelectorAll('.btn-delete').forEach(btn => {
            btn.addEventListener('click', function() {
                deleteBuilder(this.dataset.id);
            });
        });
    }
    ...
    document.getElementById('btn-add').addEventListener('click', function() {
        document.getElementById('builderModalLabel').textContent = 'Add Builder';
    ...
    // Edit builder
    function editBuilder(id, title, description) {
        document.getElementById('builderModalLabel').textContent = 'Edit Builder';
        document.getElementById('builder-id').value = id;
        document.getElementById('builder-title').value = title;
        document.getElementById('builder-description').value = description;
        new bootstrap.Modal(document.getElementById('builderModal')).show();
    }
    ...
    document.getElementById('btn-save').addEventListener('click', function() {
        ...
        const url = id ? `{{ url('/builder') }}/${id}` : '{{ url('/builder') }}';
