...
@section('title', 'Leave - Daily Log System')
...
@section('content')
<div class="pagetitle">
    <h1 class="text-capitalize">Leave</h1>
</div>

<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
        <li class="breadcrumb-item"><a href="{{ route('offwork.index') }}">Offwork</a></li>
    ...
    <div class="row">
        <div class="card">
            <div class="card-body pt-3">
                <h4 class="text-capitalize mb-3">Manage Leave</h4>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="leaveForm">
                    ...
                    <div class="mb-3">
                        <label for="leave-name" class="form-label">Name <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" id="leave-name" required>
                    </div>
                    <div class="mb-3">
                        <label for="leave-description" class="form-label">Description</label>
                        <textarea class="form-control" id="leave-description" rows="3"></textarea>
                    </div>
                </form>
            </div>
...
<script>
    ...
    function loadData() {
        ...
        fetch(`{{ url('/offwork/leave/getData') }}?data=${encodeURIComponent(JSON.stringify(params))}`)
    ...
    function renderTable(leaveTypes) {
        ...
        if (!leaveTypes || leaveTypes.length === 0) {
            tbody.innerHTML = '<tr><td colspan="4" class="text-center">No leave types found. Click "Add Row" to create one.</td></tr>';
            return;
        }

        tbody.innerHTML = leaveTypes.map((type, index) => `
            <tr>
                <td class="text-center">
                    ...
                    <i class="bi bi-x-square text-danger btn-delete" style="font-size: 18px; cursor: pointer; margin: 0 2px;" 
                       data-id="${type.id}" title="Delete"></i>
                </td>
        ...
        document.querySelectorAll('.btn-edit').forEach(icon => {
            icon.addEventListener('click', function() {
                editLeave(this.dataset.id, this.dataset.name, this.dataset.description);
            });
        });

        document.querySelectorAll('.btn-delete').forEach(icon => {
            icon.addEventListener('click', function() {
                deleteLeave(this.dataset.id);
            });
        });
    }
    ...
    document.getElementById('btn-add').addEventListener('click', function() {
        document.getElementById('leaveModalLabel').textContent = 'Add Leave Type';
    ...
    // Edit leave type
    function editLeave(id, name, description) {
        document.getElementById('leaveModalLabel').textContent = 'Edit Leave Type';
        document.getElementById('leave-id').value = id;
        document.getElementById('leave-name').value = name;
        document.getElementById('leave-description').value = description;
        new bootstrap.Modal(document.getElementById('leaveModal')).show();
    }
    ...
    document.getElementById('btn-save').addEventListener('click', function() {
        ...
        const url = id ? `{{ url('/offwork/leave') }}/${id}` : '{{ url('/offwork/leave') }}';
        ...
        .then(data => {
            if (data.success) {
                ...
                loadData();
                alert(data.message || 'Leave type saved successfully');
            } else {
    ...
    function deleteLeave(id) {
        ...
        .then(data => {
            if (data.success) {
                ...
                alert(data.message || 'Error deleting leave type');
            }
        })
        .catch(error => {
            console.error('Error:', error);
